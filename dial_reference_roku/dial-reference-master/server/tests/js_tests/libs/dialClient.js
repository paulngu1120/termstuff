...
var ssdp        = require("node-ssdp"),
    ...
    utils       = require("../libs/utils.js"),  // jvenkat - added for debug
    Q           = require("q");
...
function getApplicationStatus(host, app, clientDialVer) {
    ...
    return new Q()
      ...
      .then(function (appResourceUrl) {
          // jvenkat - the app name (YouTube) needs to be appended to the URL http://<ip:8060>/dial
          appResourceUrl += "/"; //jvenkat 
          appResourceUrl += app; //jvenkat
          appResourceUrl += clientDialVer === "2.1" ? "?clientDialVer=2.1" : "";
          //utils.printDebug("jvenkat: QUERY : URL is " + appResourceUrl);
                  ...
                  if(!error) {
                      ...
                      try {
                          ...
                          */
                          ...
                          // jvenkat start-  Roku's DIAL server returns link tag in this format: <link href="run" rel="run" />
                          // The test case assumes the link tag to be returned by DIAL in the format <link rel="run" href="run" />
                          // NOTE: AFAIK, there is no requirement in HTML to return the link attributes in a specific order 
                          if(body.indexOf("<link") !== -1) {
                              parsedResponse.href = body.split("<link href=")[1].split(" ")[0].split("/>")[0].replace(/\"/g, "");
                              if(parsedResponse.href !== "run") {
                                  return reject(new Error("@href attribute is not 'run' in the application status response"));
                              }
                              parsedResponse.rel = body.split("rel=")[1].split(" ")[0].split("/>")[0].replace(/\s/g, "").replace(/\"/g, "");
                          }
                          // jvenkat - end
...
function launchApplication(host, app, payload) {
    return new Q()
      ...
      .then(function (appResourceUrl) {
          ...
          //utils.printDebug("jvenkat: Launch: URL is " + appResourceUrl);
          if(!payload) {
...
function sleepSystem(host, key) {
    ...
    return getAppsUrl(host)
        ...
        .then(function (appsUrl) {
            //utils.printDebug("jvenkat: sleepSystem URL is " + appsUrl);
            var request = {
...
function stopApplication(host, app) {
    return new Q()
      ...
      .then(function (appResourceUrl) {
          ...
          //utils.printDebug("jvenkat: stopApplication: URL: " + appResourceUrl);
          return new Q.Promise(function (resolve, reject) {
              return httpRequest(request, function handleResponse(error, response) {
                  if(!error) {
                      return resolve(response);
    ...
    return new Q()
      ...
      .then(function (appResourceUrl) {
          ...
          return getApplicationStatus(host, app)
            .then(function (response) {
	      if(response)
                //utils.printDebug("jvenkat: hideApplication: response: ");
                //console.log(response); // jvenkat
                if(response.href) {
      ...
      .then(function (appResourceUrl) {
          ...
          //utils.printDebug("jvenkat: hideApplication URL: " + appResourceUrl);
          return new Q.Promise(function (resolve, reject) {
              return httpRequest(request, function handleResponse(error, response) {
                  if(!error) {
...
function stopApplicationInstance(instanceUrl) {
    return new Q()
      .then(function () {
          ...
          //utils.printDebug("jvenkat: stopApplicationInstance URL: " + instanceUrl);
          return new Q.Promise(function (resolve, reject) {
              return httpRequest(request, function handleResponse(error, response) {
                  if(!error) {
                      return resolve(response);
...
function hideApplicationInstance(instanceUrl) {
    return new Q()
      .then(function () {
          ...
          //utils.printDebug("jvenkat: hideApplicationInstance URL: " + instanceUrl);
          return new Q.Promise(function (resolve, reject) {
              return httpRequest(request, function handleResponse(error, response) {
                  if(!error) {
                      return resolve(response);
...
function constructAppResourceUrl(host, appName) {
    ...
    return new Q()
      ...
      .then(function (appUrl) {
          return appUrl.replace(/\/+$/, `/${appName}`); // jvenkat
      });
}

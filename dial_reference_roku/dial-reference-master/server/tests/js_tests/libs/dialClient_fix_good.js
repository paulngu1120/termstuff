                  ...
                  if(!error) {
                      ...
                      try {
                          ...
                          if(body.indexOf("<additionalData>") !== -1) {
			    							    			              utils.printDebug("JVENKAT aditonalData " );
...
function launchApplication(host, app, payload) {
    return new Q()
	  ...
	  appResourceUrl += app; //JV
	      ...
	      utils.printDebug("JVENKAT: payload is " + request.body);
          }
...
function stopApplication(host, app) {
	...
	appResourceUrl += "/"; //JV
	  appResourceUrl += app; //JV
	  	 // appResourceUrl += "/run"; //JV

		            utils.printDebug("JVENKAT STOP1: URL is " + appResourceUrl);
      ...
      .then(function (appResourceUrl) {
          var request = {
              ...
              timeout: 6000
	            ...
	            utils.printDebug("JVENKAT STOP: URL is " + appResourceUrl);
...
function hideApplication(host, app) {
    return new Q()
      ...
      .then(function (appResourceUrl) {
	   appResourceUrl += "/"; //JV
	 appResourceUrl += app; //JV
	  //  appResourceUrl += "/run/hide"; //JV

	  	  utils.printDebug("JVENKAT HIDE 1: URL is " + appResourceUrl);

          return getApplicationStatus(host, app)
            .then(function (response) {
	      
	      if(response)
	      	  	  utils.printDebug("JVENKAT HIDE  response1 is " + response.name);
...
console.log(response);
	  ...
	  utils.printDebug("JVENKAT HIDE: URL is " + appResourceUrl);
    ...
    return new Q()
      .then(function () {
          var request = {
	  	  ...
	  	  utils.printDebug("JVENKAT StopAppInstance: URL is " + instanceUrl);
      ...
      .then(function () {
          var request = {
              ...
              timeout: 6000
	  	  	  ...
	  	  	  utils.printDebug("JVENKAT HideAppInstance: URL is " + instanceUrl);
